...
| AC-018 | Example loading | [x] | built-in corpus from the committed docs (147 raw → 89 deduped `ExampleCase`), loaded via the Examples picker → `controller.loadExample` (template + sample input + expected): `packages/editor-ui/test/{examples-corpus,examples}.test.tsx`; real-engine load: `test/engine-node-adapter/test/ui/example-run.test.ts` |
