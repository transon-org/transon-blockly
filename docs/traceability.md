...
| §7.14 Component embedding | FR-102..FR-110, FR-128 | [~] | **M5 D0**: FR-105/106 callbacks carry the engine `ValidationResult`/`ExecutionResult` payloads (`packages/editor-ui/test/callbacks.test.tsx`); the three surfaces (createTransonEditor/`<transon-editor>`/`<TransonEditor>`) share one `EditorControllerOptions` config; FR-102/103/104/107/110 initial template/input, expose template, read-only, marker already wired (M4). **D1 remaining**: read-only reverse-edit gating, FR-128 chrome-only CSS-var theming (block colours stay data-driven, FR-127), FR-109 category filter, AC-026 marker round-trip |
