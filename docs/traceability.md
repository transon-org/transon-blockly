...
| AC-022 | Embeddable component | [x] | createTransonEditor() handle + <transon-editor> custom element (light DOM, change/validate/execute DOM events): `packages/editor-element/test/{create,element}.test.ts`; no engine bundled (ESM + self-contained IIFE, AD-008/020): `test/no-engine.test.ts` |
...
| §7.14 Component embedding | FR-102..FR-110, FR-128 | [ ] | component API; FR-128 chrome-only CSS-var theming (block colours stay data-driven, FR-127) — M5 D1 |
